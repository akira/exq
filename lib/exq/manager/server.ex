...
##===========================================================
  ...
  def handle_call({:enqueue, queue, worker, args, retry}, from, state) do
    Enqueuer.enqueue(state.enqueuer, from, queue, worker, args, retry)
    {:noreply, state, 10}
  end

  def handle_call({:enqueue_at, queue, time, worker, args, retry}, from, state) do
    Enqueuer.enqueue_at(state.enqueuer, from, queue, time, worker, args, retry)
    {:noreply, state, 10}
  end

  def handle_call({:enqueue_in, queue, offset, worker, args, retry}, from, state) do
    Enqueuer.enqueue_in(state.enqueuer, from, queue, offset, worker, args, retry)
    {:noreply, state, 10}
  end
